            ...
            myDats.forEach(pedido => {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    fetch(`${dominio}/api/get/${evento}/modularidades/modularidad/=/${pedido}/_/=/_`)
                        .then(data => data.json())
                        .then(data => {
                            console.log(data.MODULARIDAD[0]);
                            if (document.getElementById(data.MODULARIDAD[0]) && data.MODULARIDAD[0] != undefined) {
                                var encontrado = document.getElementById(`${data.MODULARIDAD[0]}`)
                                
                                if (encontrado.innerHTML.length <= 18) {
                                    encontrado.classList = 'encontrado'
                                    encontrado.innerHTML = `${data.MODULARIDAD[0]} - ${evento}`;
                                
                                }else{
                                    var repetido = document.createElement('p');
                                    repetido.classList = 'encontradoXn'
                                    repetido.innerHTML = `${data.MODULARIDAD[0]} - ${evento}`;
