...
Global Integer Gripper, cavity, contador_reintentos_toma
...
'		
    ...
    ElseIf fusible$ = "ATOC_15" Then			'si el mensaje es para un este fusible entonces...
		...
		P210 = ATOC15_up				    'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATOC15_load					'El punto 211 tomara los valores de los puntos load de los feeders
	...
	ElseIf fusible$ = "MAXI_50" Then			'si el mensaje es para este fusible entonces...
		...
		P210 = MAXI50_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = MAXI50_load				'El punto 211 tomara los valores de los puntos load de los feeders
...
Function tomaFusibleR							'Subrutina de toma de fusible dividida /feeder bowl/in line sencillos/in linedobles
	...
	If fusible$ = "ATO_10" Then				'si el mensaje es para este fusible entonces...
		...
		P210 = ATO10_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATO10_load					'El punto 211 tomara los valores de los puntos load de los feeders	
	...
	ElseIf fusible$ = "MAXI_30" Then		'en 1 toma de la cabidad B
		...
		P210 = MAXI30_db_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = MAXI30_db_load				'El punto 211 tomara los valores de los puntos load de los feeders
	...
	ElseIf fusible$ = "ATOC_10" Then			'si el mensaje es para este fusible entonces...
		...
		P210 = ATOC10_up				'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATOC10_load				'El punto 211 tomara los valores de los puntos load de los feeders

	ElseIf fusible$ = "ATOC_5" Then
		cilindro = cilindro_b
		Gripper = 530 					'variable del gripper, diferente para cada feeder
		P210 = ATOC5_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATOC5_load				'El punto 211 tomara los valores de los puntos load de los feeders					
	...
	ElseIf fusible$ = "ATO_20" Then			'si el mensaje es para este fusible entonces...
		...
		P210 = ATO20_up				'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATO20_load				'El punto 211 tomara los valores de los puntos load de los feeders
	...
	ElseIf fusible$ = "ATO_5" Then			'si el mensaje es para este fusible entonces...
		...
		P210 = ATO5_up				'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = ATO5_load				'El punto 211 tomara los valores de los puntos load de los feeders
...
'			Gripper = 535 	'GRIPPER LADO B
	...
	ElseIf fusible$ = "RELAY_112" Then		'si el mensaje es para este fusible entonces...
		On switch_presion
		cilindro = cilindro_a				'igualamos el valor del cilindro a cilindro correspondiente
		Gripper = 538 						'variable del gripper, diferente para cada feeder
		P210 = R112_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = R112_load					'El punto 211 tomara los valores de los puntos load de los feeders
	...
	ElseIf fusible$ = "RELAY_132" Then		'si el mensaje es para este fusible entonces...
		...
		P210 = R132_up					'El punto 210 tomara el valor de los puntos de up de los feeders
		P211 = R132_load					'El punto 211 tomara los valores de los puntos load de los feeders	
		If cavidad$ = "RELU" Then
			P210 = R132_up_RELU					'El punto 210 tomara el valor de los puntos de up de los feeders
			P211 = R132_load_RELU					'El punto 211 tomara los valores de los puntos load de los feeders	
...
Function Toma2Fusible
	...
	'shared_zone = True 'TESTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
	
	Print("esperando al otro roboto")
	If shared_zone = True Then			'si vas a usar un fusible del lado compartido...
		Do While (Sw(512) = 0)			' Sw(AVAILABLE)
		Loop
	...
	Print("Antes de Movimiento Intermedio para MAXI50, ATOC15, MINI15, ATO7.5")
	If fusible$ = "MAXI_50" Then
		Print("yendo a punto MAXI50_TOMA1ermedio")
		Go MAXI50_TOMA1
	EndIf
	If fusible$ = "ATOC_15" Or fusible$ = "MINI_15" Or fusible$ = "ATO_7.5" Then
		Print("yendo a punto INTERMEDIO")
		Go INTERMEDIO
	EndIf
	...
	On cilindro						'Activar cilindro correspondiente
...
'	Pause
	...
	contador_reintentos_toma = 0
	Do While (Sw(vacio_ok) = 0 And contador_reintentos_toma < 4)			'Esperar mientras se detecta que se tomo el fusible
		If Tmr(1) > 0.8 Then
			Print "Esperando Toma de Fusible"
			TmReset 1
		EndIf
		Print (contador_reintentos_toma)
		If Tmr(0) > 4.200 Then
			TmReset 0
			Off vacio
			Off cilindro
			...
			If Incremento < 0.5 Then
				...
				contador_reintentos_toma = contador_reintentos_toma + 1
			EndIf
			Fuse_Presence_Function
			On cilindro
			On vacio
	...
	If contador_reintentos_toma >= 3 Then
		'CONEXION TCP/IP
	...
	If fusible$ = "MAXI_40" Or fusible$ = "MAXI_30" Or fusible$ = "MAXI_50" Or fusible$ = "ATOC_5" Or fusible$ = "MAXI_50" Then
		Wait 1
	EndIf
	If fusible$ = "ATO_30" Then
		Print("esperandoooooooooooooooooooo mata")
		Wait 1
	EndIf
...
EndIf
	...
	If cavidad$ = "RELX" Or cavidad$ = "RELT" Or fusible$ = "MAXI_50" Or fusible$ = "ATO_7.5" Then
		On Gripper
		Wait 1
	EndIf

	Off cilindro						'cilindro regresando a su posicion retraida	
	If fusible$ = "ATO_25" Or fusible$ = "ATO_7.5" Then
		Wait 1
	EndIf
	
	Print("moviendose para arriba")
...
Function Abrir_Gripper
	If Sw(Gripper) Then 'si hay presencia de fusible
		On Gripper
		Wait 0.3		'tiempo para abrir gripper de fusible
	EndIf
Fend
